...
export interface IStorage {
  ...
  getCommunicationLogs(patientId: string): Promise<CommunicationLog[]>;
  createCommunicationLog(log: InsertCommunicationLog): Promise<CommunicationLog>;
  updateCommunicationLog(id: string, log: Partial<InsertCommunicationLog>): Promise<CommunicationLog | undefined>;

  // Follow-up Tasks methods
  getFollowUpTasks(clinicId: string, filters?: { doctorId?: string; isCompleted?: boolean; dueDate?: string }): Promise<FollowUpTask[]>;
  getOverdueFollowUpTasks(clinicId: string): Promise<FollowUpTask[]>;
  ...
  completeFollowUpTask(id: string, completedBy: string, notes?: string): Promise<FollowUpTask | undefined>;
}
...
export class DatabaseStorage implements IStorage {
  ...
  async getPatientFiles(patientId: string): Promise<PatientFile[]> {
    return this.db
      ...
      .where(eq(schema.patientFiles.patient_id, patientId))
      .orderBy(desc(schema.patientFiles.uploaded_at));
  }

  async createPatientFile(insertFile: InsertPatientFile): Promise<PatientFile> {
    const [file] = await this.db
      .insert(schema.patientFiles)
      .values(insertFile)
      .returning();
    return file;
  }
  ...
  async deletePatientFile(id: string): Promise<boolean> {
    await this.db
      .delete(schema.patientFiles)
      .where(eq(schema.patientFiles.id, id));
    return true;
  }
  ...
  async getCommunicationLogs(patientId: string): Promise<CommunicationLog[]> {
    return this.db
      ...
      .where(eq(schema.communicationLogs.patient_id, patientId))
      .orderBy(desc(schema.communicationLogs.created_at));
  }
  ...
  async updateCommunicationLog(id: string, updateData: Partial<InsertCommunicationLog>): Promise<CommunicationLog | undefined> {
    const [log] = await this.db
  ...
  async getFollowUpTasks(
    clinicId: string, 
    filters?: { doctorId?: string; isCompleted?: boolean; dueDate?: string }
  ): Promise<FollowUpTask[]> {
    const conditions = [eq(schema.followUpTasks.clinic_id, clinicId)];

    if (filters?.doctorId) {
      conditions.push(eq(schema.followUpTasks.doctor_id, filters.doctorId));
    }
    if (filters?.isCompleted !== undefined) {
      conditions.push(eq(schema.followUpTasks.is_completed, filters.isCompleted));
    }
    if (filters?.dueDate) {
      conditions.push(eq(schema.followUpTasks.due_date, filters.dueDate));
    }
    ...
    return this.db
      ...
      .where(and(...conditions))
      .orderBy(schema.followUpTasks.due_date);
  }
  ...
  async getOverdueFollowUpTasks(clinicId: string): Promise<FollowUpTask[]> {
    ...
    return this.db
      ...
      .from(schema.followUpTasks)
      .where(and(
        eq(schema.followUpTasks.clinic_id, clinicId),
        eq(schema.followUpTasks.is_completed, false),
        lte(schema.followUpTasks.due_date, today)
      ))
      .orderBy(schema.followUpTasks.due_date);
  }

  async createFollowUpTask(insertTask: InsertFollowUpTask): Promise<FollowUpTask> {
  ...
  async updateFollowUpTask(id: string, updateData: Partial<InsertFollowUpTask>): Promise<FollowUpTask | undefined> {
    const [task] = await this.db
      .update(schema.followUpTasks)
      .set(updateData)
      .where(eq(schema.followUpTasks.id, id))
      .returning();
    return task;
  }
...
// Export db instance for direct access (needed by analytics and other modules)
